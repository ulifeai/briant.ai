...
interface TableBlockProps {
  ...
  bodyRowContent: {
    firstCol: {
      ...
      nameProfil: string;
      hrefProfil: string;
      nameLinkProfil: string;
    };
    secondCol: string;
    thirdCol: {
      srcMemberTeam: string;
      altMemberTeam: string;
    }[];
    fourthCol: string;
    fifthCol: string;
    linkCol: {
              ...
              >
                <div className="flex flex-col justify-center items-start px-6 py-4 font-bold">
                  <div className="flex flex-row justify-center items-center gap-3">
                    <Image
                      src={bodyColContent.firstCol.srcProfil}
                      width={200}
                      height={200}
                      alt={bodyColContent.firstCol.altProfil}
                      className="h-12 w-12 rounded-full object-cover"
                    />
                    <div className="flex flex-col justify-between items-start">
                      <p className="font-medium">
                        {bodyColContent.firstCol.nameProfil}
                      </p>
                      <a
                        href={bodyColContent.firstCol.hrefProfil}
                        className="font-normal text-xs underline"
                      >
                        {bodyColContent.firstCol.nameLinkProfil}
                      </a>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-row justify-start items-center px-6 py-4">
                  {bodyColContent.thirdCol.map((image, index) => (
                    <Image
                      key={index}
                      src={image.srcMemberTeam}
                      width={24}
                      height={24}
                      alt={image.srcMemberTeam}
