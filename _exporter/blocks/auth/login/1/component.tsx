...
interface Loginprops {
  ...
  secondlabel: string;          // Added if necessary
  firstbuttontext: string;      // Added if necessary
}
...
const LoginPage = ({
  ...
  buttons,
  labels,
  secondlabel,         // Added if necessary
  firstbuttontext,     // Added if necessary
}: Loginprops) => {
  const a = "20";

  const print_something = (a: string)=>{
    // custom logic
  }
  ...
  return (
    <div className='px-[5%] h-screen'>
      ...
      <div className='flex flex-col gap-32 h-svh'>
        ...
        <div>
          <div 
            className='min-w-[40vw] flex flex-col gap-6 mx-auto'
          >
            ...
            <div>
              ...
              >
                {labels[1]} {/* Replaced secondlabel with labels[1] */}
              </div>
              <Input className='border border-black min-h-11 px-3 py-2 rounded-none'/>
            </div>
            ...
            <div className='flex flex-col gap-4'>
              <Button 
                className='rounded-none w-auto px-3 py-6 text-base'
              > 
                {labels[2]} {/* Replaced firstbuttontext with labels[2] */}
              </Button>

              <Button 
                className='text-black text-lg rounded-none w-auto px-3 py-6 bg-white border border-black flex gap-4'
              >
                {labels[3]} {/* Adjusted index accordingly */}
              </Button>
